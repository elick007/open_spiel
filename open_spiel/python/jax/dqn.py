...
class MLP(nn.Module):
  ...
  def __init__(
    ...
    Args:
        input_size (int): number of the networks' input features
        hidden_sizes (Iterable[int]): number of the networks' hidden features
        output_size (int): number of the networks' output features
        final_activation (Callable, optional): final ativation function.
          Defaults to None.
...
class DQN(rl_agent.AbstractAgent):
  ...
  def __init__(
    ...
    self._q_network = MLP(
        state_representation_size, self._layer_sizes, num_actions, seed=seed
    )
    ...
    if loss_str == Loss.MSE:
      ...
      self.loss_func = jax.vmap(
          functools.partial(optax.huber_loss, delta=huber_loss_parameter)
      )
    else:
    ...
    if gradient_clipping:
      optimizer = optax.chain(
          optimizer, optax.clip_by_global_norm(gradient_clipping)
      )

    self._optimizer = nn.Optimizer(self._q_network, optimizer, wrt=nn.Param)

    self._graphdef_q_network_opt = nn.graphdef(
        (self._q_network, self._optimizer)
    )
  ...
  def _get_jitted_update(self) -> Callable:
    ...
    def _loss_fn(
      ...
      next_q_values = jnp.where(
          legal_actions_mask, next_q_values, ILLEGAL_ACTION_LOGITS_PENALTY
      ).max(-1)

      targets = jax.lax.stop_gradient(
          rewards
          + jnp.logical_not(are_final_steps)
  ...
  def select_action(
      self, time_step: chex.ArrayTree, greedy: bool = True
  ) -> rl_agent.StepOutput:
    """Returns the action to be taken and updates the Q-network if needed.

    Args:
      time_step: an instance of rl_environment.TimeStep.
      greedy: if to take action greedily of sample it from the logits
    ...
    if (not time_step.last()) and (
      ...
      epsilon = jnp.where(
          greedy, jnp.asarray(0.0), self.epsilon_schedule(self._iteration)
      )
  ...
  def act_epsilon_greedy(self, info_state, legal_actions, epsilon):
    ...
    return self._act_epsilon_greedy(
        ...
        jnp.zeros(self._num_actions, dtype=jnp.bool)
        .at[jnp.asarray(legal_actions, dtype=jnp.int32)]
        .set(True),
        self._next_rng_key(),
        epsilon,
    )
  ...
  def add_transition(
    ...
    legal_actions = jnp.asarray(
        time_step.observations["legal_actions"][self.player_id], dtype=jnp.int32
    )
    transition = Transition(
        ...
        reward=jnp.asarray(
            time_step.rewards[self.player_id], dtype=jnp.float32
        ),
        next_info_state=jnp.asarray(
            time_step.observations["info_state"][self.player_id],
            dtype=jnp.float32,
        ),
        is_final_step=jnp.asarray(time_step.last(), dtype=jnp.bool),
        legal_actions_mask=jnp.zeros(self._num_actions, dtype=jnp.bool)
        .at[jnp.asarray(legal_actions, dtype=jnp.int32)]
        .set(True),
    )
  ...
  def learn(self) -> float:
    ...
    loss_val, new_state = self._jittable_update(
        q_network_state, target_q_network_state, transitions
    )
  ...
  def _copy_weights(
    ...
    updated_state = jax.jit(optax.incremental_update)(
        q_network_state, target_q_network_state, tau
    )
  ...
  def save(
    ...
    if save_optimiser:
      self._checkpointer.save(
          ...
          nn.state((self._q_network, self._optimizer)),
          force=True,
      )
    else:
      self._checkpointer.save(
